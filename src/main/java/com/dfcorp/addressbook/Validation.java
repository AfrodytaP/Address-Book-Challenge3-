...
public abstract class Validation {
    ...
    public static String validatePhoneNumber(String phoneNumber) {
        ...
        if (!phoneNumber.matches("^07\\d{9}$"))
            throw new IllegalArgumentException("Phone number can only contain numbers and must start with 07 and be 11 digits long");
        return phoneNumber;
    }
    ...
    public static boolean isPhoneNumberDuplicate(ArrayList<Contact> contacts, Contact contact) {
        try {
            for (Contact phoneNumber : contacts) {
                if (phoneNumber.getPhoneNumber().equals(contact.getPhoneNumber()))
    ...
    public static boolean isEmailDuplicate(ArrayList<Contact> contacts, Contact contact) {
        try {
            for (Contact email : contacts) {
                if (email.getEmail().equals(contact.getEmail()))
